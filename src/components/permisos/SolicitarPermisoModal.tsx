...
export function SolicitarPermisoModal({ userEmail, onClose, onSuccess }: SolicitarPermisoModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        if (!employeeId) { setError('No se encontro tu registro de colaborador.'); return; }
        if (!reason.trim()) { setError('Por favor describe el motivo.'); return; }
        if (!isHours && days === 0) { setError('El rango no incluye dias habiles.'); return; }
        setLoading(true);
        setError('');
        try {
                await requestPermiso({
                          employeeId, type, startDate,
              ...
              <div className="bg-[#faf8f5] border border-card-border w-full max-w-lg rounded-[2rem] p-8 shadow-2xl animate-scaleUp">
                      <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                          <h2 className="text-lg font-extrabold text-text-dark">Nueva Solicitud</h2>h2>
                                                          <p className="text-xs text-text-muted">Permiso o vacaciones</p>p>
                                            </div>div>
                                </div>div>
                      ...
                      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                                <div>
                                  <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Tipo de solicitud</label>label>
                                            <select value={type} onChange={e => setType(e.target.value as PermisosType)}
                                              ...
                                              {(Object.entries(PERMISO_LABELS) as [PermisosType, string][]).map(([val, label]) => (
                                                                              <option key={val} value={val}>{label}</option>option>
                                                                            ))}
                                            </select>select>
                                </div>div>
                        {isHours ? (
                      <div className="grid grid-cols-2 gap-3">
                                    <div>
                                                    <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Fecha</label>label>
                                                    <input type="date" value={startDate} min={today} onChange={e => setStartDate(e.target.value)}
                                                                        className="w-full bg-white border border-card-border rounded-xl px-3 py-2.5 text-sm text-text-dark focus:outline-none focus:ring-2 focus:ring-accent-gold/40" />
                                    </div>div>
                                    <div>
                                                    <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Horas</label>label>
                                                    <input type="number" value={hours} min={1} max={8} onChange={e => setHours(Number(e.target.value))}
                                                                        className="w-full bg-white border border-card-border rounded-xl px-3 py-2.5 text-sm text-text-dark focus:outline-none focus:ring-2 focus:ring-accent-gold/40" />
                                    </div>div>
                      </div>div>
                    ) : (
                      <div className="grid grid-cols-2 gap-3">
                                    <div>
                                                    <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Fecha inicio</label>label>
                                                    <input type="date" value={startDate} min={today}
                                                                        onChange={e => { setStartDate(e.target.value); if (e.target.value > endDate) setEndDate(e.target.value); }}
                                                                        className="w-full bg-white border border-card-border rounded-xl px-3 py-2.5 text-sm text-text-dark focus:outline-none focus:ring-2 focus:ring-accent-gold/40" />
                                    </div>div>
                                    <div>
                                                    <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Fecha fin</label>label>
                      ...
                      <div className="flex items-center gap-2 text-xs font-semibold text-text-muted bg-white/60 border border-card-border rounded-xl px-3 py-2">
                                    ...
                                    <span>{days} dia{days !== 1 ? 's' : ''} habil{days !== 1 ? 'es' : ''}</span>span>
                      </div>div>
                                )}
                                <div>
                                            <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Motivo</label>label>
